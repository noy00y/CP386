/*
-------------------------------------------------
Concurrent Synchrnoization Implementation using Semaphores
...
int main(int argc, char *argv[]) {
	...
	// Initialize Clock and Semaphores
	startClock();
...
{
	// Determining Thread Execution:
	int id = ((Thread*) t)->tid[2] - '0';
	int execute = 0;
	if (last_thread_id == 0 || (last_thread_id % 2 == 0 && id % 2 != 0) || (last_thread_id % 2 != 0 && id % 2 == 0)) {
		printf("Current ID: %d\n", id);
		execute = 1;
	}
	...
	if (execute) {
		sem_wait(&running); // Waiting for sem to become available and decrement count
		...
		// Exiting Thread
		sem_post(&running); // releasing sem and increment count
		last_thread_id = id;
	}
